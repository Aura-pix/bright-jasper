---
title: "The pigeonhole principle: simple math, huge consequences"
date: "2026-09-04"
tag: "Reflections"
track: "technical"
excerpt: "A deceptively simple counting idea from mathematics is quietly behind hashing, distributed systems, and a surprising amount of computer science."
readTime: "3 min read"
sample: false
keywords:
  - "pigeonhole principle explained"
  - "pigeonhole principle computer science"
  - "pigeonhole principle examples"
  - "discrete mathematics pigeonhole principle"
  - "applications of the pigeonhole principle"
  - "pigeonhole principle proof intuition"
---
...
**If you have more items than containers, at least one container must hold more than one item.** That's it. The principle guarantees that collisions are mathematically inevitable when the input space exceeds the output space.

## Why do hash collisions exist? (Exhibit A)
...
There are **infinite possible input files**, but only a **finite number of possible hash outputs** (2^256 for SHA-256, which sounds like a lot until you remember "infinite" is also a lot). More pigeons than holes. **Collisions — two different files producing the same hash — aren't a flaw in the design. They're a mathematical guarantee baked in from day one.**
...
## Why can't compression shrink every file? (Exhibit B)

Somewhere, someone is trying to build a compression algorithm that shrinks *every possible file*, no exceptions. **That person will fail consistently, and pigeonhole is why.**
...
## Why is the birthday "paradox" not a paradox? (Exhibit C)

Put **367 people in a room**. At least two of them share a birthday. Not probably. Not "pretty likely." **Guaranteed.** There are only 366 possible birthdays (yes, leap day counts), so 367 people is one pigeon too many for the holes available.
...
_If you're into more of this — deep dives into the concepts quietly running the systems we use, follow along. Next up: how Mina Protocol keeps an entire blockchain succinct using math that sounds just as "obvious" until you actually look at it._