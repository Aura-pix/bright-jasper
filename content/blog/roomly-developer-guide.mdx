...
5. [Working with schools and areas](#5-working-with-schools-and-areas)
6. [Working with housing data](#6-working-with-housing-data)
7. [Building with Roomly](#7-building-with-roomly)
8. [Handling API responses](#8-handling-api-responses)
9. [Building your integration](#9-building-your-integration)
...
- An API key. See [Authentication](#2-authentication).
...
Every request needs a valid API key sent as a Bearer token in the `Authorization` header — the exact header format is defined in the [API Specification](/blog/roomly-api-specification#authentication). What the spec doesn't cover is how to handle that key inside a real project, so that's the focus here.
...
**Keep your key private.** Never expose it in client-side JavaScript or commit it to source control. Section [9.2](#92-security-considerations) covers the architecture for keeping it server-side in a real application.
...
A `200` status with a `data` array — even an empty one — means your setup is correct. You don't need to inspect every field yet; the full response shape is covered when you get to [4.1](#41-list-properties).
...
Without filters, this returns the first page of all properties. Full response shape and pagination fields are covered in [8.1](#81-pagination).
...
This returns fields the list endpoint omits — `description` and `amenities` — which is why a detail page should always call this endpoint directly rather than reusing data from a list response. See [7.2](#72-build-a-property-details-page).
...
This is served separately from the property record because it changes far more often than the rest of a listing. Treat it as near-real-time: don't cache it, and re-check it immediately before a booking action rather than trusting an earlier response. See [9.3](#93-caching-and-stale-data).
...
Accepts the same `area`, `school`, and `propertyType` parameters used elsewhere in the API (see [4.2](#42-filter-properties)). Add `propertyType` when you want to see how a specific unit type performs within an area or school, rather than the area's blended average across all types. Full response fields are in the [API Specification](/blog/roomly-api-specification#market-data).
...
Because availability is time-sensitive (see [4.4](#44-check-availability)), avoid caching this result for longer than a few minutes, and always re-check immediately before a booking step.
...
The general rule: cache data proportionally to how expensive being wrong about it would be. Showing a stale school name costs nothing. Showing a property as available when it isn't costs the user a failed booking attempt — so treat [4.4](#44-check-availability) as close to real-time as your architecture allows.
